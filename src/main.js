...
const filtrosAdd = [
	FILTER(8,6, null, 19, 9, 1.01, 2, "Esports"),
	FILTER(10,7, null, 19, 9, 1.01, 2, "Esports"),
	FILTER(12,8, null, 19, 9, 1.01, 2, "Esports"),
	FILTER(90,85, null, 7.9, 19, 1.01, 2, null)
];
...
function waitForElementsToDisplay(selectors, callback, checkFrequencyInMs, timeoutInMs) {
...
function waitElement(selector, callbackSuccess, callbackFail, timeoutInMs) {
	var startTimeInMs = Date.now();
	(function loopSearch() {
...
const EVENT = function(tempo,competition,time1Nome,time2Nome,time1Gols,time2Gols,time1Chance,time2Chance,empateChance){
	...
	return {
		...
		tempoTotal: tempoMax,
		times: {
			um:TEAM(time1Nome,time1Gols,time1Chance),
...
const btnAposta = {
	clickFinalizar: function(apostaObj){		
		if(document.querySelector(selectors.btnFinalizar) != null){
			...
			setTimeout(function(){btnAposta.clickFinalizar(apostaObj)}, 1000);
		}
